...
const runAgriPartnerAuthVerification = async () => {
  console.log('\n================================================================');
  console.log('   AGRIMART — GENUINE AGRI_PARTNER AUTHENTICATION TEST SUITE   ');
  console.log('================================================================\n');
  ...
  try {
    ...
    // 2. Login with Phone Number + Correct Password
    console.log('\n▶ [TEST 2]: Logging in with Phone Number + Password...');
    ...
    console.log(`  ✅ TEST 2 PASSED: Phone+Password login succeeded. Role: ${loggedInUser.role} (JWT Role: ${decodedToken.role})`);

    // 3. Reject Incorrect Password with HTTP 401
    console.log('\n▶ [TEST 3]: Verifying incorrect password rejection (HTTP 401)...');
    ...
    if (wrongPassRes.statusCode === 401 && !wrongPassRes.body.token) {
      console.log(`  ✅ TEST 3 PASSED: Incorrect password properly rejected with HTTP 401.`);
    } else {
      throw new Error(`Security failure: Incorrect password was not rejected with 401. Status: ${wrongPassRes.statusCode}`);
    }

    // 4. Phone Normalization (+91 prefix, leading zero, spaces)
    console.log('\n▶ [TEST 4]: Testing Phone Number normalization (+91 prefix, spaces)...');
    ...
    if (normalizedRes.statusCode === 200 && normalizedRes.body.user?.role === 'AGRI_PARTNER') {
      console.log(`  ✅ TEST 4 PASSED: Phone normalization succeeded. Role: ${normalizedRes.body.user.role}`);
    } else {
      throw new Error(`Phone normalization test failed: ${JSON.stringify(normalizedRes.body)}`);
    }

    // 5. Authenticated Profile Verification (GET /api/auth/me)
    ...
    console.log(`  ✅ TEST 5 PASSED: Session profile retrieved for '${meRes.body.user.name}'. Role: ${meRes.body.user.role}`);

    // 6. Role-Based Access Control on Agri Partner Endpoint
    console.log('\n▶ [TEST 6]: Verifying Role Authorization (/api/auth/agri-partner-only)...');
    ...
    if (roleRes.statusCode === 200 && roleRes.body.success) {
      console.log(`  ✅ TEST 6 PASSED: Agri Partner successfully authorized. Role: ${roleRes.body.user.role}`);
    } else {
      throw new Error(`Role authorization failed: ${JSON.stringify(roleRes.body)}`);
    }

    // 7. Phone-Based Password Recovery & Reset Flow
    console.log('\n▶ [TEST 7]: Testing Phone-based Password Reset flow...');
    ...
    if (newLoginRes.statusCode === 200 && newLoginRes.body.user?.role === 'AGRI_PARTNER') {
      console.log(`  ✅ TEST 7 PASSED: Password reset & login with new password succeeded. Role: ${newLoginRes.body.user.role}`);
    } else {
      throw new Error(`Login with new password failed: ${JSON.stringify(newLoginRes.body)}`);
    }

    console.log('\n================================================================');
    console.log('  🎉 ALL AGRI_PARTNER AUTHENTICATION TESTS PASSED!               ');
    console.log('================================================================\n');
