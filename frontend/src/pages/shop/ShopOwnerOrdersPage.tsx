...
export const ShopOwnerOrdersPage: React.FC = () => {
  ...
  const handleUpdateStatus = async (
    ...
    setErrorMsg(null);
    try {
      const res = await updateOrderStatusApi(orderId, newStatus, reason);
      if (res.success) {
        setSuccessMsg(`Order ${orderNumber} updated to ${newStatus}.`);
        if (rejectModalOrder) {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    if (activeTab === 'IN_PROGRESS') {
      return ['ACCEPTED', 'PROCESSING', 'PACKED', 'DISPATCHED'].includes(order.status);
    }
